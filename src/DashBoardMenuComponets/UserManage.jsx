...
// Centralize the API base URL (use Vite env var if you have one set up,
// falling back to your deployed backend)
...
function UserManage() {
  ...
  // Build an authorized axios instance so every request carries the admin's token
  const getAuthHeaders = () => {
    const token = localStorage.getItem("token"); // adjust key if you store it elsewhere
    return {
      headers: {
  ...
  const fetchUsers = async () => {
    ...
    setError(null);
    try {
      const res = await axios.get(`${API_BASE}/api/auth/users`, getAuthHeaders());
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${API_BASE}/api/auth/users/${id}`, getAuthHeaders());
      fetchUsers();
    } catch (err) {
  ...
  if (error) {
    return (
      <div className="p-6 bg-gray-100 min-h-screen">
        <div className="bg-[#1a1a1a] text-red-400 shadow-md rounded-xl p-6">
          <p>{error}</p>
          <button
            onClick={fetchUsers}
